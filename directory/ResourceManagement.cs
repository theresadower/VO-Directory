...
namespace OperationsManagement
{
    ...
    public class ResourceManagement
    {
        ...
        private static string strGetHarvestInfo = "select top 1 harvestedFrom, harvestedFromID from resource where ivoid = '$1' order by harvestedfromdate desc";
        ...
        private static validationStatus SetValidationLevels(string identifier)
        {
            validationStatus status = new validationStatus();
            XmlDocument doc = new XmlDocument();
            status += ResourceManagement.GetExistingResource(identifier, ref doc, false);
            bool bChanged = false;
            if (status.IsValid)
            {
                try
